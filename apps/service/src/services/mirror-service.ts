...
export function createMirrorService(mirrorRoot: string) {
  const syncMap = new Map<string, SyncRecord>();
  const syncIndexPath = join(mirrorRoot, ".sync-index.json");
  ...
  function agentDir(slug: string): string {
    return join(mirrorRoot, "Agents", slug);
  }
  ...
  mkdirSync(join(mirrorRoot, "Agents"), { recursive: true });
  if (!existsSync(join(mirrorRoot, "README.md"))) {
    writeFileSync(
      join(mirrorRoot, "README.md"),
...
`,
  ...
  return {
    mirrorRoot,
    /** @deprecated Use mirrorRoot. Kept for backward compat during rename. */
    get vaultRoot(): string { return mirrorRoot; },
...
export type MirrorService = ReturnType<typeof createMirrorService>;

/** @deprecated Use MirrorService / createMirrorService. */
export type VaultService = MirrorService;
/** @deprecated Use createMirrorService. */
export const createVaultService = createMirrorService;
